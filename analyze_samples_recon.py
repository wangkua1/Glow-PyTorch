...
def get_prior(model,batch_size, clamp=False):
...
def generate_from_noise(model, batch_size,clamp=False, guard_nans=True):
    randz = get_prior(model,batch_size, clamp=clamp)
...
def main(kwargs):
    ...
    for idx in tqdm(idxs):
        ...
        # x_train_p = run_pgd(x_train, f_loss).detach()
        # # x_train_p = x_train
        # with torch.no_grad():
        #     bpd = f_loss(x_train)
        #     bpd_p = f_loss(x_train_p)
        # # ipdb.set_trace()
        # plot_bpds(bpd_p, bpd, os.path.join(kwargs['output_dir'], f'train_bpds_{idx}.png'))
        # plot_samples(x_train, os.path.join(kwargs['output_dir'], f'train_{idx}.png'))
        # plot_samples(x_train_p, os.path.join(kwargs['output_dir'], f'train_{idx}_p.png'))
        # plot_samples(x_train_p - x_train, os.path.join(kwargs['output_dir'], f'train_{idx}_d.png'))
