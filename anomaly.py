...
DATA_DIR  = os.path.join('/h/wangkuan/projects/uncertain-few-shot-master', 'data/')
...
def _load_ood_dataset(ood_dataset_name, opt_config):
    ...
    # 3x32x32
    elif ood_dataset_name == 'gaussian3':
        return np.random.normal(size=(opt_config['n_anom'], 3, 32, 32)) * opt_config['ood_scale']
    elif ood_dataset_name == 'uniform3':
        return np.random.uniform(size=(opt_config['n_anom'], 3, 32, 32)) * opt_config['ood_scale']
    elif ood_dataset_name == 'rademacher3':
        return (np.random.binomial(1, .5, size=(opt_config['n_anom'], 3, 32, 32)) -.5)*2 * opt_config['ood_scale']
    elif ood_dataset_name == 'gaussian84':
        return np.random.normal(size=(opt_config['n_anom'], 3, 84, 84)) * opt_config['ood_scale']
    elif ood_dataset_name == 'uniform84':
        return np.random.uniform(size=(opt_config['n_anom'], 3, 84, 84)) * opt_config['ood_scale']
    elif ood_dataset_name == 'rademacher84':
        return (np.random.binomial(1, .5, size=(opt_config['n_anom'], 3, 84, 84)) -.5)*2 * opt_config['ood_scale']
    elif ood_dataset_name == 'texture3':
        return torch.load(os.path.join(DATA_DIR, 'dtd.t7')) / 256.
    elif ood_dataset_name == 'places3':
        return torch.load(os.path.join(DATA_DIR, 'places.t7')) / 256.
    elif ood_dataset_name == 'svhn':
        ds = torchvision.datasets.SVHN('.', split='test', transform=None, target_transform=None, download=True)
        return ds.data[:10000] / 256.
...
def load_ood_data(ooc_config):
    ...
    # Load OOD dataset
    ood_dataset = _load_ood_dataset(ooc_config['name'], ooc_config)
    ood_dataset = torch.FloatTensor(ood_dataset[:ooc_config['n_anom']])
    if ooc_config['cuda']:
        ood_dataset = ood_dataset.cuda()
    return ood_dataset
